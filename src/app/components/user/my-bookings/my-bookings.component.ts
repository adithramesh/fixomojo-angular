...
export class MyBookingsComponent implements OnInit, OnDestroy {
    ...
    this.subscription.add(
      this.bookingService.bookingUpdated$
        ...
        .subscribe((updatedBooking: TableData) => {
          const index = this.bookingsTableData.findIndex(
            (b) => b.id === updatedBooking.id
          );
          if (index !== -1) {
            this.bookingsTableData[index].bookingStatus =
              updatedBooking.bookingStatus;
            this.bookingsTableData[index].isCompleted =
              updatedBooking.isCompleted;
          }
        })
    );
  }
      ...
      next: (response: BookingResponse) => {
        ...
        if (response.success && response.bookingList?.items) {
          this.bookingsTableData = response.bookingList.items.map((booking) =>
            this.mapBookingsToTableData(booking)
          );
  ...
  private prepareAndOpenCancelModal(booking: TableData): void {
    ...
    const timeDifferenceInHours =
      (timeSlotStart.getTime() - now.getTime()) / (1000 * 60 * 60);
    ...
    // }
      ...
      this.cancelModalMessage =
        'This booking cannot be cancelled with a refund as it is within 24 hours of the service time. Are you sure you want to proceed?';
      this.cancelButtonText = 'Cancel Booking';
    }
  }
  ...
  confirmCancelBooking(): void {
    ...
    this.bookingService
      .cancelBooking(this.bookingToCancel.id as string)
      .subscribe({
        next: (response) => {
          if (response.success) {
